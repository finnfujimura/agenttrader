...
def _write_parquet_dataset(base: Path) -> None:
...
def test_build_index_and_streaming_adapter(tmp_path: Path):
    _write_parquet_dataset(tmp_path)
    index_path = tmp_path / "backtest_index.duckdb"

    first = build_index(force=True, data_dir=tmp_path, index_path=index_path)
    assert first["ok"] is True
    assert first["skipped"] is False
    assert first["stats"]["markets_indexed"] > 0

    second = build_index(force=False, data_dir=tmp_path, index_path=index_path)
    assert second["ok"] is True
    assert second["skipped"] is True

    third = build_index(force=True, data_dir=tmp_path, index_path=index_path)
    ...
    assert len(ids) > 0
    market_id, platform = ids[0]
    assert platform == "polymarket"

    points = list(adapter.stream_market_history(market_id, platform, 1700000000, 1710000000))
    assert len(points) > 0
    ...
    assert len(bars) > 0
    assert [p.timestamp for p in bars] == sorted(p.timestamp for p in bars)

    latest = adapter.get_latest_price_before(market_id, platform, 1704067200)
    assert latest is None or 0.0 <= latest <= 1.0
    adapter.close()


def test_build_index_missing_dataset_returns_error(tmp_path: Path):
